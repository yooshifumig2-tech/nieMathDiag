(function () {
  ...
  const filters = [
    ...
    { key: "review", name: REVIEW, label: "13·14复习练习" }
  ];
  const PAGE_SIZE = 8;
  const givenSteps = {
  ...
  const hashKey = () => {
    ...
    return raw.includes("review") ? "review" : (raw.match(/13|14|15|16/) || ["13"])[0];
  };
  ...
  function qcard(q, index) {
    ...
    const selfCheck = q.chapter.includes("第16章") ? "检查每个系数、符号、指数和括号，再用展开或逆运算交叉验证。" : "把结论代回条件，检查点名、角的对应顺序、严格不等号、垂直条件或全等式顺序。";
  ...
  function report() {
    ...
    const groupField = filter === REVIEW ? "sourceChapter" : "point";
    const title = filter === REVIEW ? "第13·14章综合复习报告" : `${filter}练习报告`;
    return `<section class="hero"><div><span class="kicker" style="color:#d9d3ff">${filter === REVIEW ? "复习讲义主线 · 独立记录" : "章节练习报告"}</span><h1>${title}</h1><p>掌握度只根据必做题中已经提交的题目计算；未作答不会被算作错误。培优题单独记录，不改变基础掌握率。</p></div><div class="hero-stats"><span><b>${currentStats.pct}%</b>必做掌握</span><span><b>${currentStats.done}/${currentStats.all}</b>必做进度</span><span><b>${challengeDone.length}/${challengeRows.length}</b>培优完成</span><span><b>${challengeRight.length}</b>培优答对</span></div></section>
      <section class="card" style="margin-top:22px"><span class="kicker">${filter === REVIEW ? "按原章节查看证据" : "分知识点掌握度"}</span><div class="report-grid">${meterCards(rows, groupField)}</div>${filter === REVIEW ? `<h3 style="margin-top:24px">分板块掌握度</h3><div class="report-grid">${meterCards(rows, "section")}</div>` : ""}<div class="report-summary"><b>当前建议</b><p>${masteryBand(currentStats.pct, currentStats.done, currentStats.all)}。完成全部必做题后再把此百分比作为阶段性掌握证据。</p></div><div class="footer-actions"><button class="button" onclick="print()">导出 / 保存PDF</button><a class="button" href="index.html?view=mindmap">查看同步后的思维导图</a><button class="button primary" data-view="questions">返回练习</button></div></section>`;
  }

  function render() {
    const isReview = filter === REVIEW;
    const chapterNav = document.querySelector?.('.course-nav a[href="math-practice.html"]');
    const reviewNav = document.querySelector?.('.course-nav a[href="math-practice.html#review"]');
    chapterNav?.classList.toggle("active", !isReview);
    reviewNav?.classList.toggle("active", isReview);
    if (view === "report") {
    ...
    app.innerHTML = `<section class="hero"><div><span class="kicker" style="color:#d9d3ff">${isReview ? "以复习讲义为主 · 与原练习分别保存" : "基础 → 中等 → 提高"}</span><h1>${isReview ? "第13·14章<br>复习练习" : "第13—16章<br>分层练习"}</h1><p>${isReview ? "用24道必做题系统复盘三角形与全等三角形，再用4道培优题完成综合迁移。" : "每课时必做题覆盖教学流程，章末提高题用于综合迁移；提交后提供四步解析、动态图与AI追问。"}</p></div><div class="hero-stats"><span><b>${currentStats.done}/${currentStats.all}</b>必做进度</span><span><b>${currentStats.pct}%</b>已答掌握</span><span><b>${challenges}</b>培优挑战</span><span><b>逐题</b>图解与AI</span></div></section>
