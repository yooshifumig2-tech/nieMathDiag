...
- `/semester-review.html`：20个复习专题，知识要点、易错辨析、四步例题、40道专题检测与笔记。
- `/semester-practice.html`：100道独立练习（84道必做、16道提高），分章筛选、错题回练、24题独立综合检测和学习报告。
- 以六份复习讲义的知识与题型为依据，改编条件并复核答案；科学记数法按正确的系数范围表述。
- 新页面对每条答案、完成标记、笔记、位置分别保存；同步写入本机双份记录，使用IndexedDB作为另一份本机副本。失败记录显示提示并重试，位置保存不会隐藏答案的失败状态。
- 支持进度JSON导出与合并导入；不同章节的旧记录继续保留。清除整个网站数据仍可能删除所有本机副本，因此提供可下载备份，不声称云端保存。
- 无需新增依赖，Netlify仍直接发布`public`目录。
...
- 人教版八年级上册第13—18章交互学习，共48个课时：苏格拉底选择题、动态推演、即时检测与章末分层练习
- 独立的第13、14章、第15、16章及第17、18章复习练习：以所附复习讲义为主，分别保存进度并生成分板块掌握报告
...
- 实时记忆当前章节、课时、题目、解析步骤和阅读位置；刷新或意外退出后可继续，并保留双份本机备份
- 首次完整打开后缓存学习页面与题库，网络中断时仍可继续学习和作答（AI辅导需联网）
- 不收集姓名、学校、性别
