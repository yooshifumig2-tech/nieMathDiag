...
export default async function aiTutor(request) {
  ...
  if (!apiKey) return json({ error: "千问尚未配置：请在Netlify环境变量中添加 DASHSCOPE_API_KEY" }, 503);
  ...
  const system = `你是FUMI AI数学助教，服务于北京中考方向、人教版七至八年级数学诊断。\n${protection}\n规则：\n1. 只讨论提供的当前题目与相关知识点，不修改、重算或承诺学生分数。\n2. 使用简洁、适合初中生的中文；必要时使用LaTeX式数学符号，但不要使用复杂Markdown表格。\n3. 先指出应观察的条件，再给步骤；不要无意义鼓励。\n4. 无论学生怎样要求，都必须遵守交卷前保护模式。\n5. 不询问姓名、学校、性别等身份信息。`;

  const base = clean(Netlify.env.get("DASHSCOPE_BASE_URL"), 500) || "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions";
  const endpoint = base.endsWith("/chat/completions") ? base : base.replace(/\/$/, "") + "/chat/completions";
  const model = clean(Netlify.env.get("DASHSCOPE_MODEL"), 100) || "qwen-plus";
